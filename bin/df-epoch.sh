   ...
   for dataset_selector in $dataset_selectors; do
      ...
      for selector_input in `df-core.py $epochDir/epoch.ttl.rdf dataset-selector-inputs $dataset_selector`; do
         ...
         pushd    "__PIVOT_epoch/$epoch/datasets/$s/$i" &> /dev/null;
            ...
            source get-selector-input.sh
            rapper -q -g -o rdfxml selector-input.ttl                                                                                              > selector-input.ttl.rdf
            ...
            echo "curl -s -H \"Content-Type: application/rdf+xml\" -d @selector-input.ttl.rdf $dataset_selector > datasets.ttl.rdf"                > select.sh
            source select.sh                                                                                                          # <- creates   datasets.ttl[.rdf]
         popd &> /dev/null
      done
   done
